...
async function transTempPicOne(img, dir, id, isCheck = true) {
  if (isCheck) {
    ...
    // 先获取文件信息，检查下大小
    // 使用 Promise 包装 getFileInfo
    const fileSizeCheckRes = await new Promise((resolve, reject) => {
      fs.getFileInfo({
        filePath: img,
        success(fileInfoRes) {
          console.log("file info res:", fileInfoRes);
          const fileSize = fileInfoRes.size; // 文件大小，单位：B  
    ...
    if (!fileSizeCheckRes) {
      wx.hideLoading();
      return pageHelper.showModal('图片大小不能超过2M', '温馨提示');
    }
    // 读取文件的二进制数据  
    console.log("file path:", img);
    const readFileRes = await new Promise((resolve, reject) => {
      fs.readFile({
        filePath: img,
        encoding: 'base64', // 指定编码为二进制  
        success(readRes) {
          const fileBufferBase64 = readRes.data; // 文件的二进制数据  
          console.log('File buffer base64:', fileBufferBase64);
    ...
    if (!readFileRes) {
      wx.hideLoading();
      return pageHelper.showModal('获取图片内容失败', '温馨提示');
    }
    let check = await contentCheckHelper.imgCheckCloud(readFileRes);
