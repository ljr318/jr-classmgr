...
async function lock(lockedKey, expireTime = 60, lockedOwner = '') {
  ...
  try {
    ...
    if (error.errCode === 'DATABASE_DUPLICATE_KEY') {
      console.error('唯一索引冲突，获取锁失败:', error, lockedKey, lockedOwner, expireTime);
      return 1;
    } else {
...
async function unlock(lockedKey, lockedOwner = '') {
  const where = {
    LOCKED_KEY: lockedKey,
    LOCKED_OWNER: lockedOwner
  };
  return await del('bx_dist_lock', where);
}
