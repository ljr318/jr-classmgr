...
class AdminMeetController extends BaseProjectAdminController {
  ...
  async getAdminMeetList() {
    ...
    for (let k = 0; k < list.length; k++) {
      ...
      list[k].MEET_START_TIME_STR = timeUtil.timestamp2Time(list[k].MEET_START_TIME);
      list[k].MEET_END_TIME_STR = timeUtil.timestamp2Time(list[k].MEET_END_TIME);
  ...
  async getUserJoinList() {
    ...
    let service = new AdminMeetService();
    let result = await service.getUserJoinList(input);
  ...
  async getMeetDetail() {
    ...
    let service = new AdminMeetService();
    let detail = await service.getMeetDetail(input._id);
    return detail;
  }
